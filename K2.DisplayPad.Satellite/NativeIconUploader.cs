...
internal static class NativeIconUploader
{
    ...
    /// </summary>
    public static bool Upload(string imagePath, int buttonIndex, uint deviceId)
    {
        try
        {
            ...
            const int size = 102;
            using var resized = ResizeImage(source, size, size);
            using var canvas = new Bitmap(size, size, PixelFormat.Format32bppRgb);
            ...
            using (Graphics g = Graphics.FromImage(canvas))
            {
                ...
                path.AddArc(canvas.Width - cornerRadius, 0, cornerRadius, cornerRadius, 270f, 90f);
                path.AddArc(canvas.Width - cornerRadius, canvas.Height - cornerRadius, cornerRadius, cornerRadius, 0f, 90f);
                path.AddArc(0, canvas.Height - cornerRadius, cornerRadius, cornerRadius, 90f, 90f);
            ...
            int outIndex = 0;
            // canvas is exactly size×size (32bpp, no row padding since size*4 is a multiple of 4),
            // so every pixel is in range — no need for BC's rectangle-containment check here.
